...
class DataQualityMetricsCalculator:
    def __init__(self, scheme_name: str, validator: DataModelValidator | None = None):
    ...
    def calculate_sections(self) -> dict[str, pd.DataFrame]:
        ...
        - 0_ISSUES: validator.issues_df with columns: rule, desc, owner, table, column, constraint_name, length, limit
        - 1_SCHEME_MEASURES
        - 2_MODEL_MEASURES
        - 3_MODEL_INDICATORS
        """
        ...
        # ---------------------------------------
        # 2) MODEL MEASURES from validator.issues_df
        # ---------------------------------------
        df_count = (
        ...
        for code, desc, (num_code, den_code) in indicator_specs:
            ...
            df_indicators_rows.append((code, desc, f"{value:.2f}%"))

        df_indicators = build_section_df(df_indicators_rows)

        return {
            "0_ISSUES": df_issues,
            "1_SCHEME_MEASURES": df_scheme,
            "2_MODEL_MEASURES": df_measures,
            "3_MODEL_INDICATORS": df_indicators,
        }
